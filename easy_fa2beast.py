...
# vresion 2.0
# The update  make it sensiable to transform the single-copy gene obtained from the Orthofinder to the format readable in BEAST2(nex).
...
MAFFT = '/public/home/huangyj/anaconda3/envs/compare_g/bin/mafft'
RAxML = '/public/home/huangyj/anaconda3/envs/compare_g/bin/raxmlHPC-PTHREADS'
ASTRAL = '/public/home/huangyj/software/Astral/astral.5.7.8.jar'
TRIMAL = '/public/home/huangyj/anaconda3/envs/compare_g/bin/trimal'
#####################################################################################
...
def get_SingleGeneSeq(SingleGeneIDs, allseqs):
	...
	os.makedirs("SingleGene", exist_ok=True)
	os.chdir("SingleGene")
...
def MSA_SingleCopyGene(SingleGeneIDs):
	...
	'''
	os.makedirs("SingleGene_MSA", exist_ok=True)
	...
	for item in SingleGeneIDs:
		...
		cmd1 = MAFFT + ' --quiet' + ' ../SingleGene/' + seqfile + ' >' + seq_aln_file
		run_command(cmd1)
...
def merge_SingleCopyGene(SingleGeneIDs):
	...
	with open("merged_allSingleGenes.fas","w") as fh:
		for key,value in mydict3.items():
			fh.write(">" + key + "\n" + "".join(value) + "\n")
	SeqIO.convert("merged_allSingleGenes.fas", "fasta", "merged_allSingleGenes.nexus", "nexus", "protein")
	print("Convert fasta to nexus")

def MLtree_Concatenation():
	'''
	Construct the ML species tree with the concatenate methold
	'''
	os.makedirs("Concatenation", exist_ok=True)
...
def MLtree_Coalescence(SingleGeneIDs):
	...
	'''
	os.makedirs("Coalescence", exist_ok=True)
	...
	cmd4 = 'java -jar ' + ASTRAL +' -t 2 -i allSingleGenes_tree.nwk -b allSingleGenes_bootstrap.txt -r ' + nb + ' -o Astral.coalescent_out.result'
	run_command(cmd4)
